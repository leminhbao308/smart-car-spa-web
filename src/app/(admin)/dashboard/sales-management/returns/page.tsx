...
import {useCreateReturn, useFullfilledOrders, useReturnedOrders} from "@/lib/api/hooks";
import {SaleOrderLineResponse, SaleOrderResponse, SaleReturnResponse} from "@/lib/api";
...
const ReturnsPage = () => {
  ...
  const [selectedOrderForReturn, setSelectedOrderForReturn] = useState<SaleOrderResponse | null>(null);
    ...
    return returnedOrders.filter((order) => {
      const matchesSearch =
        ...
        (order.sales_order?.customer?.full_name &&
          order.sales_order?.customer.full_name.toLowerCase().includes(searchText.toLowerCase())) ||
        (order.sales_order?.customer?.phone_number &&
          order.sales_order?.customer.phone_number.includes(searchText));

      const matchesStatus = statusFilter === "all" || order.sales_order.status === statusFilter;
      ...
      render: (_: any, record: SaleReturnResponse) => (
        <div>
          {record.sales_order?.customer ? (
            <>
              <div style={{fontWeight: 500}}>{record.sales_order?.customer.full_name}</div>
              ...
              <Table
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    width: 130,
                    render: (_: any, record: SaleOrderLineResponse) => {
                      const total = record.quantity * Number(record.unit_price);
                      return (
                        <Text strong style={{color: "#ff4d4f"}}>
              ...
              <div style={{textAlign: 'right'}}>
                <Text strong style={{fontSize: 16}}>
                  ...
                  <Text style={{color: '#ff4d4f', fontSize: 18}}>
                    ₫{calculateOrderTotal(selectedOrderForReturn).toLocaleString()}
                  </Text>
                </Text>
              </div>
            </>
