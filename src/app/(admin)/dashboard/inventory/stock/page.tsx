...
import StockBatchModal from "@/components/ui/Modal/StockModal/StockBatchModal";
import StockTransactionHistoryModal from "@/components/ui/Modal/StockModal/StockTransactionHistoryModal";
...
const StockInventoryPage = () => {
  ...
  const [stockBatchVisible, setStockBatchVisible] = useState(false);
  ...
  const [exportBranch, setExportBranch] = useState<string | undefined>(
    undefined
  );
  ...
  const fetchInventoryLevels = async () => {
    ...
    try {
      ...
      const stockItems: StockTableItem[] = products.map((product: Product) => {
        ...
        const lastPurchasePrice = getLastPurchasePrice(
          product.product_id,
          selectedBranchId
        );
        const sellingPrice = sellingPrices[product.product_id];
        const profitMargin =
          lastPurchasePrice && sellingPrice
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      await InventoryService.exportStockReport(date, exportBranch);
  ...
  const columns: ColumnsType<StockTableItem> = [
    ...
    },
    {
      ...
      key: "reserved",
      width: 90,
      render: (qty: number) => <div style={{ color: "#faad14" }}>{qty}</div>,
    },
    {
      title: "Khả dụng",
      dataIndex: "available",
  ...
  const handleTransactionHistory = (record: StockTableItem) => {
    setSelectedProduct(record.product);
    setTransactionHistoryVisible(true);
  };

  const handleStockBatch = (record: StockTableItem) => {
    setSelectedProduct(record.product);
    setStockBatchVisible(true);
  };
  ...
  const isLoading =
    loading || inventoryHook.loading || branchesLoading || pricingHook.loading;
  ...
  return (
    <div>
      ...
      {/* Stock Batch Modal */}
      <StockBatchModal
        visible={stockBatchVisible}
        onClose={() => {
          setStockBatchVisible(false);
          ...
          >
            <div style={{ fontSize: 12, color: "#0369a1" }}>
              ...
              <ul style={{ marginTop: 8, marginBottom: 0, paddingLeft: 20 }}>
                <li>
                  Báo cáo sẽ bao gồm tất cả sản phẩm trong kho tính đến ngày
                  chọn
                </li>
