...
const BranchesPage = () => {
  ...
  const {branches, loading, error, refreshBranches} =
    useBranchesByCenter(centerId);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (type: string) => (
        <Tag
          color={
            type === "PREMIUM" ? "gold" : type === "VIP" ? "purple" : "blue"
          }
        >
      ...
      render: (record: BranchDisplay) => (
        <div>
          ...
          <div style={{fontSize: 11, color: "#8c8c8c"}}>
            {Math.round(record.utilization_rate | 0)}% sử dụng
          </div>
        </div>
      ),
    },
