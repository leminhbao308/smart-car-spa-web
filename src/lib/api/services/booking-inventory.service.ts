...
export class BookingInventoryService {
   ...
   */
  static async getBookingRequiredProducts(
    booking: BookingInfoDto
  ): Promise<BookingInventoryItem[]> {
    console.log(
      "🔍 Getting required products for booking:",
      booking.booking_id
    );
    ...
    try {
      ...
      for (const bookingItem of booking.booking_items) {
        ...
        console.log(
          `🔍 Processing service: ${bookingItem.service_id} (${bookingItem.item_name})`
        );

        // Get service details with products
        const service: Service = await ServiceService.getServiceById(
        ...
        if (
          ...
          console.log(
            `ℹ️ Service ${service.service_name} has no required products`
          );
          continue;
        }
        ...
        for (const serviceProduct of service.service_products) {
          ...
          console.log(
            `✅ Added required product: ${inventoryItem.productName} (${inventoryItem.quantity} ${inventoryItem.unit})`
          );
        }
      }

      console.log(
        `📊 Total required products for booking ${booking.booking_id}:`,
  ...
  static async releaseInventoryForBooking(
    ...
    try {
      ...
      if (requiredProducts.length === 0) {
        console.log(
          "ℹ️ No products to release for this booking - services may not have required products"
        );
        return;
      }
