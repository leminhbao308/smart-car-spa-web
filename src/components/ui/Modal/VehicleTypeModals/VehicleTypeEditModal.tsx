...
const VehicleTypeEditModal: React.FC<VehicleTypeEditModalProps> = ({
  ...
  useEffect(() => {
    if (typeData && visible) {
      form.setFieldsValue({
        ...
        fuelEfficiency: typeData.fuelEfficiency,
        comfort: typeData.comfort,
        performance: typeData.performance,
      });
      setCharacteristics(typeData.characteristics || []);
      setTargetCustomers(typeData.targetCustomers || []);
      setPopularBrands(typeData.popularBrands || []);
      setExamples(typeData.examples || []);
    }
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      message.error("Có lỗi xảy ra khi cập nhật loại xe!");
    } finally {
      setLoading(false);
    }
  };
      ...
      title={
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <Title level={4} style={{ margin: 0 }}>
            Chỉnh sửa loại xe: {typeData.typeName}
          </Title>
        </div>
      }
      open={visible}
